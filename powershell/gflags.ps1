...
param (
    ...
    [Parameter()][switch]$server                       # switch to input a list of servers/clusters
)
...
Write-Host "Checking $vip Gflags now."
...
function setGflag($servicename, $flagname, $flagvalue, $reason){
    if($clear){
        ...
        write-host "setting  $($servicename):  $flagname = $flagvalue on $vip"
    }
    $gflagReq = @{
...
foreach($service in $gflags){
    ...
    foreach($serviceGflag in $serviceGflags){
        ...
        $gflaglist += @{'clusterName' = $($cluster.name); 'serviceName' = $svcName; 'flagName' = $serviceGflag.name; 'flagValue' = $serviceGflag.value; 'reason' = $serviceGflag.reason; 'timestamp' = $timeStamp}
    }
}

$gflaglist = ($gflaglist | ConvertTo-Json -Depth 99 | ConvertFrom-Json)
$gflaglist | Export-Csv -Path "gflags/gflags-$($cluster.name).csv" -Encoding utf8 -NoTypeInformation

...
param (
    ...
    [Parameter(Mandatory = $True)][string]$jobPrefix,  # name of the job to add Object to
    [Parameter()][string]$startTime = '16:00', # e.g. 23:30 for 11:30 PM
    [Parameter()][string]$timeZone = 'America/Los_Angeles', # e.g. 'America/Los_Angeles'
    [Parameter()][string]$storageDomainName = 'DefaultDatalockDomain',  # storage domain you want the new job to write to
    [Parameter(Mandatory = $True)][string]$policyName,  # protection policy name
    [Parameter()][switch]$paused,  # pause future runs (new job only)
    [Parameter()][int]$incrementalSlaMinutes = 720,
    [Parameter()][int]$fullSlaMinutes = 1440,
...
$logtoday = Get-Date -UFormat '%m-%d-%Y'
$logFile = "$($jobPrefix)-log-$logtoday.txt"
...
while($unprotectedCount -gt 0){
    ...
    if($jobs){
        ...
        if($job.office365Params.objects.Count -ge $maxObjectsPerJob){
            ...
            Write-Host "New job name is $jobName, gathering objects to add"

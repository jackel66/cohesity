...
# headings
$headings = "Cluster Name`tSource Name`tAgent Version`tOS Type`tOS Name`tStatus"
...
$nodes.rootNodes | Sort-Object -Property {$_.rootNode.ProtectionSource.name} | `
         Select-Object -Property @{label='Source Name'; expression={$_.rootNode.physicalProtectionSource.name}},
                                 ...
                                 @{label='Status'; expression={
                                    if ($_.registrationInfo.refreshErrorMessage){
                                        $_.registrationInfo.refreshErrorMessage
                                    } else {
                                        "Healthy"
                                    }
                                }}
...
foreach ($node in $nodes.rootNodes){
    ...
    # Check if registrationInfo and refreshErrorMessage exist
    $agentError = if ($node.registrationInfo.refreshErrorMessage) {
        $node.registrationInfo.refreshErrorMessage
    } else {
        "Healthy"  # If it's empty or Not Present assume Healthy
    }

    "$clusterName`t$name`t$version`t$hostType`t$osName`t$agentError" | Out-File -FilePath $outfileName -Append
}

<#
.SYNOPSIS
    Automatically add O365 objects to a protection job based on a prefix.

.DESCRIPTION
    This script will automatically add O365 objects to a protection job based on a prefix. It will discover all O365 objects and add them to the specified job. If the job does not exist, it will create a new one.
    The script can be run in dry-run mode to see what would be added without actually making any changes.


Author:
    Brian Seltzer - Cohesity Professional Services
#>
...
param (
    ...
    [Parameter()][switch]$DryRun  # <-- Added DryRun switch
)
...
if($thesejobs){
    if ($DryRun) {
        Write-Host "DRYRUN: Would append to $logFile"
    } else {
...
while($unprotectedCount -gt 0){
    ...
    foreach($objectId in $unprotectedIndex){
        ...
        if($job.office365Params.objects.Count -lt $maxObjectsPerJob){
            ...
            if ($DryRun) {
                Write-Host "DRYRUN: Would add $($unprotectedName["$($objectId)"]) to job"
            } else {
    ...
    if($newJob){
        if ($DryRun) {
            Write-Host "DRYRUN: Would create protection job $jobName"
        } else {
        ...
        if ($DryRun) {
            Write-Host "DRYRUN: Would update protection job $($job.name)"
        } else {
...
if($allObjectsAdded -gt 0){
    if ($DryRun) {
        Write-Host "DRYRUN: Would log $allObjectsAdded $objectString added"
    } else {

...
if ($export -eq "accounts" -or $export -eq "all") {
    ...
    $exportData | ConvertTo-Json -Depth 10 | Out-File -FilePath "$($vip).account_export.json" -Encoding utf8
    Write-Host "✅ Exported $($exportUsers.Count) users and $($exportGroups.Count) groups to $($vip).account_export.json"
}
